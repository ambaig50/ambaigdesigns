...
export default async function handler(req, res) {
  ...
  const tokenRes = await fetch(
    `https://graph.threads.com/v18.0/oauth/access_token?client_id=${process.env.META_CLIENT_ID}&redirect_uri=${process.env.REDIRECT_URI}&client_secret=${process.env.META_CLIENT_SECRET}&code=${code}`
  );
  ...
  res.send("Threads connected with long-lived token!");
}
